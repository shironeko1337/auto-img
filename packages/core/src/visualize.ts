...
>(
  ...
  /**
   * A visualizer function that take an image in a certain state and then
   * 1. set the current state
   * 2. initial render the image instantly
   * 3. show an animation of how the image is transited from the previous state
   *    to current state
   * 4. wait for the next action or end async function
   */
  return async (
    image: Image,
    message: string = "",
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (
        controllerConfig.setContainerBackground &&
        renderTarget === "container"
      ) {
    ...
    if (!prevMessage) {
      prevMessage = await context.renderMessage(message);
    } else {
      prevMessage = await context.animateMessage(prevMessage, message!);
    }
...
type DOMControllerConfig = {
  ...
  // true: set background for container and set calculated position and size
  // false: only set background for image and stretch to fill
  setContainerBackground?: boolean;
};
...
class DOMController extends Controller {
  ...
  private inputs: {
    ...
    setContainerBackground: HTMLInputElement;
  };
  ...
  constructor(context: DOMContext) {
    ...
    panel.innerHTML = `
      <div class="config-panel">
        ...
        <div class="config-panel-grid">
          ...
          <div><label><input type="checkbox" id="setContainerBackground"></label></div>
    ...
    this.inputs = {
      ...
      setContainerBackground: this.configPanel.querySelector(
  ...
  getConfig(): [TestInput, ControllerConfig] {
    return [
      ...
      },
      {
        ...
        setContainerBackground: this.inputs.setContainerBackground.checked,
      },
    ];
  }
...
> {
  ...
  async renderRect(
    ...
    el.style.transitionDelay = `${this.animationDuration / 1000}s`;
